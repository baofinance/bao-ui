...
import useBasketRedeem from '../../../hooks/useBasketRedeem'
...
const WithdrawModal: React.FC<WithdrawModalProps> = ({
	...
	const { onBasketRedeem } = useBasketRedeem(nid, redeemToWeth)
	// TODO: make one contract that can redeem all baskets
	const redeemAllowance = useAllowancev2(
		basketContract.options.address,
		bao.getContract('basketRedeem').options.address,
		pendingTx
	)
	const { onApprove: onApproveRedeem } = useApprovev2(
		basketContract,
		bao.getContract('basketRedeem'),
	)
	...
	return (
		<Modal>
			...
			<ModalTitle text={`Redeem ${basketName}`} />
			<TokenInput
