		...
		<div className={classNames('', className)}>
			...
			<div className='realtive flex min-h-fit min-w-fit flex-1 flex-col rounded'>
				{stats.map(({ label, value }) => (
					<div className='grid grid-cols-2 break-words rounded px-3 py-3 last:pb-1' key={label}>
						<Typography variant='base' className='my-auto align-middle font-bakbak text-baoWhite'>
							{label}
						</Typography>
						<Typography variant='base' className='my-auto text-end font-bakbak text-baoWhite'>
							{value}
						</Typography>
					</div>
				))}
