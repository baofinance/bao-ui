...
export const useAccountBalances = (): Balance[] => {
  ...
  const underlyingTokens = Config.markets.map((market) => market.underlyingAddresses[Config.networkId])
  
  const [balances, setBalances] = useState<Balance[] | undefined>()
  
  const fetchBalances = useCallback(async () => {
    const data: Contract[] = underlyingTokens.map((address) => bao.getNewContract('erc20.json', address))
    ...
    setBalances(Object.keys(multicallResults).map((address) => ({
      address,
