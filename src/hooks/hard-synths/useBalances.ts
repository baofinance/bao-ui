...
export const useAccountBalances = (): Balance[] => {
  ...
  const fetchBalances = useCallback(async () => {
    const data: Contract[] = tokens.map((address) => bao.getNewContract('erc20.json', address))
      ...
      await bao.multicall.call(
        MultiCall.createCallContext(
          data.map((contract) => ({
            ref: contract.options.address,
            contract,
            calls: [{ method: 'symbol' }, { method: 'decimals' }, { method: 'borrowBalanceStored', params: [account] }],
          }))
        )
      ),
    )
