...
const useHealthFactor = () => {
  ...
  const fetchHealthFactor = useCallback(async () => {
    ...
    const balanceRes = Multicall.parseCallResults(
      await bao.multicall.call(balanceQuery),
    )

    if (Object.keys(balanceRes).length === 0)
      return setHealthFactor(new BigNumber(0))
