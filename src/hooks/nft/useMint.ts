...
export const useElderClaimedCheck = () => {
	...
	const fetchWhitelistClaimed = useCallback(async () => {
		const _isClaimed = await getNFTWhitelistClaimed(getElderContract(bao), account)
...
export const useBaoSwapClaimedCheck = () => {
	...
	const fetchWhitelistClaimed = useCallback(async () => {
		const _isClaimed = await getNFTWhitelistClaimed(getBaoSwapContract(bao), account)
		setIsClaimed(_isClaimed)
	}, [bao, account])
