...
const getPriceHistoryMultiple = async (tokenAddresses: string[], networkId = 1, first?: number): Promise<any> =>
	await _querySubgraph(_getPriceHistoryQueryMultiple(tokenAddresses, first), 'sushiExchange', networkId)

const getPriceHistory = async (tokenAddress: string, networkId = 1): Promise<any> =>
